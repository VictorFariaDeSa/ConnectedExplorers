...
class RobotsMathNode(Node):
    ...
    def listener_callback(self, msg, robot_index):
        ...
        robot_list = [self.robots_instances[robot] for robot in self.robots_list]
        self.math_handler.refresh_laplacian_matrix(robot_list)
    ...
    def on_map_cb(self,msg):
        ...
        self.math_handler = MathHandler(
            self.map_handler,
            self.max_robots_dist,
            len(self.robots_list),
            self.min_dist_to_wall,
            self.max_dist_to_wall
        )
