...
class SingleRobotControllerNode(Node):
    ...
    ****************************************************************************
    ...
    def Get_robot_specifict_gradient_values(self):
        return self.gradient_vector[(self.robot_number-1)*DIMENSIONS:(self.robot_number)*DIMENSIONS]
    ...
    def get_optimized_movement_vector(self, ideal_vector):
            ...
            p_curr = np.array([p_curr_raw.x, p_curr_raw.y, p_curr_raw.z])[:DIMENSIONS].reshape(DIMENSIONS, 1)
            ...
            for robot_name in self.robots_list:
                ...
                p_other = np.array([p_other_raw.x, p_other_raw.y, p_other_raw.z])[:DIMENSIONS].reshape(DIMENSIONS, 1)
    ...
    def get_collision_safe(self, safe_dist):
        ...
        for i in range(len(poses)):
            for j in range(i + 1, len(poses)):
                d = np.sqrt((poses[i].x - poses[j].x)**2 + 
